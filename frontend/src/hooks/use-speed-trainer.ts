...
export function useSpeedTrainer() {
  ...
  const validate = (): string | null => {
    if (startTempo < 50 || endTempo > 100 || endTempo <= startTempo) {
      return "Invalid inputs to speed trainer. Please ensure that both tempo values are between 50 and 100% and starting tempo is less than ending tempo";
    }
    if (step <= 0 || reps < 1) {
