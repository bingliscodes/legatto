...
- **Per-IP rate limit** — a custom Redis fixed-window counter (**5/hr + 10/day**) as a route dependency; the real client IP comes from uvicorn `--forwarded-allow-ips` (behind nginx, `request.client.host` is otherwise *nginx's* IP → global-by-accident).
...
- **The per-IP limit is sized *relative to the global cap*, not in isolation** (revised **2026-07-16**, tightened from 10/hr + 30/day → **5/hr + 10/day**). The original numbers were picked on their own merits — "30 songs a day is plenty for one person" — which is the wrong frame. Set against a 65/day global cap, **one IP could consume nearly half the day's entire budget**, so the per-IP limit wasn't buying fairness, it was just a slower path to the same exhaustion. At 10/day it takes **≥7 distinct IPs** to exhaust the cap. **The ratio between the two limits is the actual design parameter** — a per-key limit only means something as a fraction of the global one, and neither number should be chosen without the other in view.
...
### D16 — Pre-separated demo track (`is_demo` as read-authorization + CC-licensed demo content)

**Date:** 2026-07-16

**Problem — the conversion funnel, not the wait.** The entire product sits behind an upload. A hiring manager arriving from a résumé link **has no audio file to hand** — to see the app do anything they'd have to go find music, download it, come back, and upload. Most won't, so the project's centerpiece showed *nothing* to most of the people it exists to impress. Worse, the economics were backwards: every visitor who *did* upload cost ~$0.015 of GPU and one of the 65 daily slots (D14) — **paying for the ones who converted and showing nothing to the ones who didn't.** (Note the wait itself was never the issue — RunPod separates a 4-min song in ~1 min, and the `processing → completed` row is honest UX. The original 5–10 min figure is the *CPU* path.)

**Design — one `Track` row flagged `is_demo`.** `get_tracks` filters `(Track.user_id == user_id) | Track.is_demo`; `get_track` lets the demo through the ownership check. Stems live at `{track_id}/stems/` exactly like any other track, so `list_stems` / `url_for` / presigned URLs all work unchanged.

**Key decisions + why:**
- **No demo-ingestion path — reuse the real pipeline.** The demo was uploaded *through the live app*, separated by the normal Celery→RunPod job, then the row was promoted. Zero new code, GPU paid once. Every alternative considered (hand-seeding stems into Spaces, copying objects per user, a separate demo bucket) amounted to rebuilding a pipeline that already exists.
- **A boolean column, over a "demo user" or a dedicated `/tracks/demo` endpoint.** The column says what it means (no magic UUID in config to decode later), and the demo arrives as a *normal track in a fresh visitor's library* — which is the behavior wanted — instead of needing a second frontend path into the player. Cost: one Alembic revision.
- **⚠️ `is_demo` is an authorization flag, not a label — and nothing in the name says so.** It is the single bit that makes a row **world-readable**, deliberately defeating D12's per-user isolation for exactly one row. Consequence: `UPDATE tracks SET is_demo = true` with a forgotten `WHERE` **publishes every user's uploads to every visitor**, with no record of which row was meant to be the demo (i.e. not cleanly reversible). Promotions therefore run inside an explicit transaction, verifying the row count reads `UPDATE 1` before `COMMIT`. **Any future admin surface that sets this flag inherits the same property.**
- **CC-licensed content over an own recording (for now).** Recording it himself is the zero-risk option and gives control over how guitar-forward the demo is — but it's a real time sink gated on writing something decent, and asking a teacher for samples introduces an **async dependency on someone else's schedule** while the goal was to ship the same day. Since `is_demo` makes the demo swappable with one `UPDATE`, **the first demo is disposable** → ship CC now, swap later, stop optimizing it. **NB: recording yourself playing someone else's song does *not* solve licensing** — you own the performance, not the composition.
- **The track:** *"Holistic Thought (Pensamiento Holístico)"* by **David J. Barrios** ([FMA](https://freemusicarchive.org/music/david-j-barrios/single/holistic-thought-pensamiento-holistico-progressive-post-metal/), **CC BY 4.0**). Progressive post-metal was chosen deliberately — it's **guitar-forward**, so the `htdemucs_6s` guitar stem (the whole rationale for the 6-source model in D2) actually demonstrates the technical choice instead of being buried.
- **The modification notice is a license requirement, not a courtesy.** CC BY 4.0 §3(a)(1)(B) requires indicating if the material was modified, and **stem separation is unambiguously an adaptation.** Attribution lives in the pre-existing (and until now unused) `artist` column — which already renders in `TrackList` — as `"David J. Barrios — CC BY 4.0, separated into stems"`, doing creator + licence + modification notice in the one field that's actually visible. Full TASL (Title/Author/Source/Licence) with working links lives in the README.

**Deferred (logged, not built):** a **delete path** (none exists yet — when it does, `is_demo` rows need a guard, or a visitor nukes the shared stems for everyone); a **partial unique index** (`CREATE UNIQUE INDEX … ON tracks (is_demo) WHERE is_demo`) to make "exactly one demo" a database invariant rather than a hand-enforced convention. Until then, **swapping the demo needs both `UPDATE`s in one transaction** (un-flag old, flag new) or there's a window where visitors see two demos or none.

**Hard-won lessons.**
- **`default=` vs `server_default=` — Python-side vs database-side, and only one exists during `ALTER TABLE`.** `default=False` is applied by the ORM at INSERT; it emits no DDL. So adding a `Mapped[bool]` (⇒ NOT NULL) column to a *populated* table needs `server_default`, or Postgres rejects it — existing rows have no value to put there. And `server_default` takes **SQL**, not a Python bool (`ArgumentError: … got 'bool'`), because it's rendered *into* the DDL string; SQLAlchemy refuses to guess since the literal is dialect-dependent (`false` on Postgres, `0` on MySQL/SQLite). Use `sa.false()`. Alembic's `script.py.mako` already imports `sqlalchemy as sa` into every generated migration.
- **SQLAlchemy's `|` binds tighter than `==`.** Python won't let `and`/`or` be overloaded (they short-circuit and coerce to `bool`), so SQLAlchemy hijacks the *bitwise* operators — and inherits their precedence, which is wrong here. `Track.user_id == user_id | Track.is_demo` silently parses as `Track.user_id == (user_id | Track.is_demo)`. **Every comparison in a boolean filter needs its own parens.**
...
**Key decisions + why:**
- **react-joyride over a hand-rolled tour.** Overlay, spotlight, positioning, and Next/Back are the whole job and are fiddly to get right (scroll, resize, focus). A ~2-decision feature isn't worth reimplementing that. Originally shipped on v2.9.3, whose peer range predates React 19 — **upgraded to v3.2.0 on 2026-09-10** (`react: "16.8 - 19"`), see the amendment below.
...
- **Uncontrolled mode via the `useJoyride` hook** (joyride owns the step index; we only observe). The help menu opens the tour at an arbitrary step with `controls.start(n)`, and a late-mounting anchor is handled by `options.targetWaitTimeout` rather than by us. _(Originally controlled mode — we owned `stepIndex` — so a not-yet-loaded target would simply wait. v3 removed the behaviour that rested on; see the amendment.)_
- **Help button bypasses the gate without clearing it.** Replaying calls `controls.start()` but never removes the `seen` flag, so a replay doesn't re-arm the first-visit auto-launch.
...
**Hard-won lessons.**
- **joyride's default button text is light, and `primaryColor` is light in dark mode** (`--primary` = `oklch(0.922)`), so the Next button washed out (light-on-light). Fix: pair it with the dark `--primary-foreground`, matching the app's own `bg-primary text-primary-foreground` buttons. (v3 derives the primary button's text colour from `options.backgroundColor`, so the same washout recurs — the override just moved from `styles.buttonNext` to `styles.buttonPrimary`.)

**Deferred:** automated tests — no frontend test harness exists yet (see Pending).

---

#### D17a — amendment (2026-09-10): react-joyride v2 → v3, and why the tour went uncontrolled

**Problem — the tour broke the deploy.** v2.9.3's peer range stops at React 18, so `npm ci` inside the frontend Docker build failed with `ERESOLVE` on React 19 (Actions run 34405810397). The build had always been installed locally with `--legacy-peer-deps`; CI's `npm ci` re-validates peers and doesn't take the flag from anywhere but a config file. First fix was a `frontend/.npmrc` carrying `legacy-peer-deps=true` — which works, but silences peer-conflict detection for _every_ frontend dependency, not just this one. v3.2.0 declares `react: "16.8 - 19"`, so upgrading removes the conflict at source and the flag is gone.

**Why the mode changed.** The original controlled-mode rationale — "a not-yet-loaded target simply waits, because joyride re-anchors when `loaded` flips" — **is not reproducible on v3 in either mode.** v3 polls for a step's target for `options.targetWaitTimeout` (default **1s**) and then stops for good; the `TARGET_NOT_FOUND` path changes no state in controlled mode, and its effect depends only on joyride's internal state, so a parent re-render when `loaded` flips will not revive it. Controlled mode would freeze on the missing step with no exit but the X. Uncontrolled at least advances past it. Given the wait is now carried by `targetWaitTimeout` (set to **15s**, comfortably longer than a slow stem fetch+decode) either way, uncontrolled is strictly better on the failure path — and it lets `use-tour.ts` drop the `run`/`stepIndex` state and the step arithmetic entirely.

**Migration notes (v2 → v3 is a rewrite, not a bump):** default export → named `Joyride` / `useJoyride`; `callback` → `onEvent(data, controls)`; `CallBackProps` → `EventData`; `styles.options.*` → a top-level `options` prop; `styles.buttonNext` → `styles.buttonPrimary`; `showProgress`/`showSkipButton`/`disableOverlayClose` → `options.showProgress` / `options.buttons` / `options.overlayClickAction`; per-step `disableBeacon` → `skipBeacon`; and `status === FINISHED || SKIPPED || action === CLOSE` collapses to a single `EVENTS.TOUR_END`. v3 also drops `react-floater`/`react-is@16` for floating-ui.

**Hard-won lesson — in v3, "close" means _advance_, not _end the tour_.** On v2 the X emitted `ACTIONS.CLOSE` and our own handler decided that meant "finish"; the semantics were ours to define. On v3 they're the library's: `controls.close()` does `index + 1`, so in uncontrolled mode the X silently walked the user to the next step. Ending the tour is `skip` — it's what sets `STATUS.SKIPPED` and fires `TOUR_END`. Two settings follow from that:
- `options.closeButtonAction: "skip"` — points the X at the right action.
- `options.dismissKeyAction: false` **plus an app-level Escape listener** calling `controls.skip()`. `dismissKeyAction` only accepts `'close' | 'next' | 'replay' | false`; there is no `'skip'`, so every value it does take either advances or replays. `controls.skip()` self-guards on `status !== RUNNING`, so a plain `document` keydown listener is inert outside the tour and doesn't collide with the welcome modal's own Escape handling.

**The Skip button was dropped** (`options.buttons: ["back", "close", "primary"]`) — once the X ends the tour, Skip did the identical thing, and two controls for one action is just noise. The X is the single exit. Its `locale.close` is "Close (Esc)", which surfaces the keyboard exit as the button's `title` and `aria-label`; joyride's `CloseButton` renders a hardcoded `svg` glyph and its own JSX children override the spread, so there is no config path to visible text beside that icon short of replacing the whole tooltip component.
...
- **DAU read-surface** (D15) — tracking is built **and deployed** (worker running embedded beat via `-B`; the nightly snapshot is collecting). Still no endpoint surfacing today's `SCARD` + the history rows. **Deliberately deprioritized 2026-07-16:** the number is a `psql` query away, the portfolio value of D15 is the *design* (unique counts, zero per-request DB writes) rather than a dashboard, and a public counter reading "4" would actively hurt. Build the display when there's something worth displaying.
