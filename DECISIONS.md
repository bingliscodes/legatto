...
3. **Containerize + droplet** 🔨 — ✅ *containerize half done (2026-07-03)*: slim backend image (demucs lazy-imported so the prod image ships **no torch**; base `requirements.txt` vs dev-only `requirements-local.txt`), **one image serves both api + celery worker** (compose `command:` override), multi-stage frontend image (Node build stage → `nginx:alpine` serving the built SPA). `compose.prod.yaml` runs the full five-service topology locally — **nginx (sole front door, `:80`) + api + worker + redis + self-hosted Postgres 17 (named `pgdata` volume)** — verified end-to-end through nginx (upload → RunPod separation → stems in Spaces → play). Same-origin `/api` (**backend owns the prefix**; nginx `proxy_pass` pass-through, no trailing slash; Vite dev proxy mirrors it); secrets via compose `${VAR}` interpolation from a gitignored root `.env` (never in the committed yaml or the image); migrations run by hand via `alembic upgrade head`. 🔨 *remaining*: provision the droplet by hand (Docker + secrets + bring up), DNS (`legatto.live`), TLS (Let's Encrypt).
