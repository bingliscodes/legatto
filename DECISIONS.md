...
4. **CI/CD** 🔨 *(next)* — GitHub Actions build → registry → droplet pull & restart. Automate the proven manual deploy; also removes the droplet's need to authenticate *to* GitHub (retires the hand-managed deploy key).
...
6. **Production deploy** 🔨 nearly done (D11) — **LIVE at `https://legatto.live`.** Serverless GPU (RunPod) + object storage (DO Spaces) + Celery/Redis + self-hosted Postgres + self-served nginx SPA on a DigitalOcean droplet (Docker Compose), HTTPS via Let's Encrypt (auto-renewing). Build steps 1–3 ✅; **only step 4 (CI/CD via GitHub Actions) remains** to turn the `git pull` + `up -d` deploy into a single `git push`. Full detail + hard-won lessons in D11.
